...
class alarmPlugin(Plugin):
    ...
    localizations = {
		...
		"de-DE": u"Setze Wecker\u2026"
            }, "alarmWasSet": {
                "en-US": "Your alarm is set for {0}:{1} {2}.",
		"de-DE": "Dein Wecker ist gesetzt auf {0}:{1} {2}.",
            }, "alarmSetWithLabel": {
                "en-US": "Your alarm {0} {1} is set for {2}:{3} {4}.",
		"de-DE": "Dein Wecker ist auf {0} {1} ist gesetzt auf {2}:{3} {4}."
            }
        }
    }
    ...
    res = {
        'setAlarm': {
            ...
            'de-DE': '.*wecker auf.* (0?[1-9]|1[012]).*([0-5]\d)?\s?(morgen|nachmittag)\s?(\bname\b)?\s?(([a-z0-9]{1,7}\s)?([a-z0-9]{1,7})\s?([a-z0-9]{1,7}))?'
    ...
    def setAlarm(self, speech, language):
        ...
        if (alarmAMPM == "pm" or alarmAMPM == "nachmittag" and alarmHour != 12):
            alarm24Hour += 12
