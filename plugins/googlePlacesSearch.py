...
class googlePlacesSearch(Plugin):
     @register("en-US", "(find|show|where).* (nearest|closest) (.*)")
     @register("de-DE", u"(wo|zeige).* (ist|in der nähe) (.*)")
     def googleplaces_search(self, speech, language, regex):
          ...
          if jsonString != None:
               ...
               if (response['status'] == 'OK') and (len(response['results'])):
                    ...
                    self.say(u"Entschuldigung, ich habe nichts für: "+str(Title)+" in deiner Nähe gefunden!")
          else:
